...
const schemaLocationsLocal = 'schema/';

const schemaOutputDirectory = schemaLocationsLocal + 'out/';
...
try {
  for (let schema of schemaFileNames) {
    ...
    let outputPath = schemaOutputDirectory + schemaFileNames[schema].substring(0, schemaFileNames[schema].length-4) + 'compiled.json'
